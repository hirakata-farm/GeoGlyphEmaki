...
const getPoints = (map, points = [], { isLatLng, isValue } = {}) => {
  if (isLatLng) {
    return points.map(([lat, lng, value]) => {
      var point = map.latLngToLayerPoint(L.latLng(isValue ? lat : lng, isValue ? lng : lat));
...
const getPoints2 = (map, points = [], offset, zoom, { isLatLng, isValue } = {}) => {
  if (isLatLng) {
    return points.map(([lat, lng, value]) => {
	var ppoint = map.project(
	    L.latLng(isValue ? lat : lng, isValue ? lng : lat),
	    zoom
	).round();
	var point = ppoint.subtract(offset);
      return isValue ? [point.x, point.y, value] : [point.x, point.y];
    });
  } else {
    return points;
...
L.WebGlTemperatureMapLayer = L.Layer.extend({
  ...
  setPoints: function(points = [], options = { isLatLng: false, draw: true }) {
    if (this.tempMap && this._map) {
      ...
      this.initialoffset = this._map.getPixelOrigin();
      this.initialzoom = this._map.getZoom();
      var _points = getPoints(this._map, points, { isValue: true, isLatLng: options.isLatLng });
  ...
  // -------------------------------------------------------------
  updatePoints: function(points = [], options = { isLatLng: false, draw: true }) {
    if (this.tempMap && this._map) {
      var _points = getPoints2(this._map, points, this.initialoffset, this.initialzoom ,{ isValue: true, isLatLng: options.isLatLng });
      this.tempMap.set_points(_points);
      options.draw && this.needRedraw();
      return this;
    }
