...
let instance = 0;
function TemperatureMapIdw(options = {}) {
  let _options = {};
...
TemperatureMapIdw.is_supported = function() {
  let canvas = document.createElement('canvas');
  let context = canvas.getContext('webgl', { stencil: true });
  return canvas && context && context.getExtension('OES_texture_float');
};

TemperatureMapIdw.prototype.set_points = function(points, low_val, high_val, normal_val, { mask = false } = {}) {
...
TemperatureMapIdw.prototype.init_buffers = function() {
  if (!this.context) return;
...
TemperatureMapIdw.prototype.init_shaders = function() {
  if (!this.context) return;
...
TemperatureMapIdw.prototype.draw = function({ scale = 1.0, transform = [0.0, 0.0] } = {}) {
  if (!this.context) return;
  let w = this.canvas.width;
  let h = this.canvas.height;
...
TemperatureMapIdw.prototype.hide_points = function() {
...
let oldScale = 1;
TemperatureMapIdw.prototype.debug_points = function(scale, offset) {
  this.hide_points();
...
TemperatureMapIdw.prototype.resize = function(width = this.canvas.clientWidth, height = this.canvas.clientHeight) {
